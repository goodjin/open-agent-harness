...
| 04 | `04-session-state-and-lifecycle.md` | Session 状态、Turn 生命周期、恢复、终态和状态投影。 |
...
| 08 | `08-observability-and-projection.md` | `session_result`、状态投影、outbox、日志和调试证据。 |

## v2 文档原则

- 先讲协作模型，再讲字段。
- 先定义对象，再定义对象之间的关系。
- 先讲稳定语义，再讲 DB、文件、事件等实现投影。
- 区分模型可见协议、运行时内部协议、持久化事实和外部可读投影。
