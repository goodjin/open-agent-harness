...
本文档是 Open Agent Harness 的治理协议总纲。它定义 Harness 要治理什么、Runtime 拥有什么控制权、Agent 如何被约束、不同详细协议文档如何挂接到同一套对象模型上。

模型侧 DSL 协议见 `docs/harness-protocol/03-model-runtime-protocol.md`。Agent 基础模型见 `docs/harness-protocol/01-agent-model-and-authoring.md`。Workflow 体系在本协议中定位为 durable orchestration adapter，详见 `docs/harness-protocol/08-workflow-durable-orchestration-adapter.md`。

## 治理愿景

Harness 的目标不是只运行一个 coding agent，而是建立一个可治理的 agent operating environment。

这个环境里的核心要求是：

- Agent 是小而明确的执行者，有身份、输入契约、输出契约、工具边界和权限边界。
- Tool call 是受 Harness 管理的结构化 action，不是模型随意发出的副作用。
- 多 Agent 协作通过 Runtime 控制的 Command、Event、Projection、Gate 和 Assignment 发生，而不是通过 Agent 之间直接聊天发生。
- 用户和 UI 能观察、暂停、恢复、审查、批准和追溯 agent work 的路径。
- Runtime 能把短期工具编排、长期 workflow、review loop、memory maintenance、release gate、human approval 和 future automation 统一投影成可审计的 run state。
...
Harness 的长期目标是通用 DSL：模型声明 semantic action graph，Runtime 执行、校验、存储、投影和恢复。这个目标不要求第一版就脱离实际模型能力。

协议支持多种模型侧承载路径：

- **DSL Declaration**：模型输出 Harness DSL declaration，Runtime 将 declaration 解析成 action、assignment、event 和 projection。
- **ToolCall Carrier**：v1 可以用模型稳定遵循的 toolCall 通道承载协议入口，例如 `AgentProtocolOutput` 或等价 runtime-owned protocol tool。
- **Recovery Normalization**：当模型输出可安全识别的 tool request 或 delegation request 时，Runtime 可以把它归一化成协议 action，再走同一套校验、权限、日志和结果投影。
- **Rejection Boundary**：如果 Runtime 无法唯一确定 executor、参数、side effect 或权限，必须阻断执行，要求模型重试协议格式或请求用户/Decision Actor 决策。

toolCall 是 v1 的稳定承载方式，不是最终抽象边界。所有承载路径进入 Runtime 后都必须变成相同的 Harness action 表示。

Workflow 只是本协议的一种应用形态，不是协议边界。协议还应该支持研究流、审查循环、记忆维护、长期监控、发布门禁，以及其他不适合单一 Workflow 表达的 Agent 集合。

## 关联规范文档

主协议文档使用数字编号。支持性研究和评测材料放在 `docs/harness-protocol/support/`，不作为规范源。

| 编号 | 文档 | 定位 |
|---|---|---|
| 00 | `00-harness-governance-protocol.md` | 总纲：治理愿景、核心对象、状态控制、门禁、记忆、概念、决策等级。 |
...
| 04 | `04-action-executor-contract.md` | Action、Executor、toolCall carrier 和 recovery normalization。 |
| 05 | `05-routing-and-delegation-policy.md` | Agent routing、delegation、child session trace 和 handoff。 |
| 06 | `06-state-event-projection-model.md` | Event source、Projection、状态映射、事务边界和 replay。 |
| 07 | `07-context-memory-visibility-policy.md` | Context Bundle、Memory Scope、data visibility、privacy 和 replay policy。 |
| 08 | `08-workflow-durable-orchestration-adapter.md` | Workflow 作为 DSL 之上的 durable orchestration adapter，包含 Runner agent 和实现计划。 |
| 09 | `09-ui-console-and-agent-management.md` | 用户如何管理、观察和使用 Harness 系统。 |
...
- 可以写入 `packages/harness/src/workflow/**`
...
替换概念必须解释：是什么新事实、新需求、新失败或新约束让 previous concept 不再充分。纯偏好式修改应该被拒绝或升级。

例子：

- 新 provider 需要额外字段
- 测试证明 previous assumption 是错的
- 隐私约束让 previous storage design 失效
...
- “state transition path not verified”
...
```json
{
  ...
  "scope": {
    "read": ["packages/harness/src/auth/**"],
    "write": ["packages/harness/src/auth/**", "packages/harness/test/auth/**"]
  },
  ...
  "acceptance": [
    ...
    "Auth callers receive the same success behavior and a typed timeout failure.",
...
```json
{
  ...
  "changes": [
    {
      "path": "packages/harness/src/auth/session.ts",
      "kind": "modified",
  ...
  "assumptions": [
    "The configured timeout is the source of truth.",
...
```json
{
  ...
  "included": [
    ...
    "packages/harness/src/auth/session.ts"
  ],
  "excluded": [
    "unrelated workflow branches",
    "raw transcripts from other actors"
  ],
  "summary": "Auth refresh waits on provider refresh without a hard timeout guard."
}
```
...
Projection 表示由事件推导出的当前操作真相。比如 previous API 名已经是 historical，新 API 名才是 active。
...
```json
{
  ...
  "refs": ["packages/harness/src/auth/profile.ts"],
  "replaces": "user-profile-api@1",
  "new_information": "The profile API requires provider metadata that is not available from the previous profile contract.",
...
当 Concept 被替换后，Runtime 应触发 impacted reference scanning，并在代码、文档或测试仍引用 previous concept 时创建后续任务。
