...
Task binding preloads that immutable content snapshot, then owns one immediate SQLite transaction for assignment verification and Task routing. Inside the transaction, Runtime re-reads the complete assignment row fingerprint and requires the same active identity, running status, source type, source session, Run, action, content reference, version, and hash both before and after the Task write. A concurrent supersede or content-version change therefore rolls back the complete binding. Successful create/update routing consumes the Assignment as completed in that same transaction; exact source-locator replay returns the existing Task or draft Revision, while later ordinary Runs continue the current Task without recreating it. Handoff remains active until its persisted proposal workflow exists, and ordinary execution is rejected with a pending-handoff conflict during that interval. Delegated child binding applies the same guard while also reading the child's parent session in that transaction; callers provide only the parent/session/Run/action locator, while Task title and body come from the canonical assignment snapshot.
