...
A confirmed package may contain only the approval action. The next executable package without a confirm action binds the unique current confirmed Assignment by its persisted source locator and registers the new execution Run; it does not reconstruct Task content from the later declaration or legacy fallback. If delegated Task binding fails after child creation, Runtime marks the child Assignment failed, settles the delegation through the normal failed-result path, removes it from parent pending fan-in state, and leaves the child terminal with audit logs. These compensation steps are idempotent and preserve the failed records rather than deleting them.
