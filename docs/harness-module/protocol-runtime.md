...
Session Task provides the single-task read model above Runs. A current Revision can bind one internal Run through `workflow.run_id`; Runtime only projects actions, progress, and a persisted model outcome from that explicit binding, so an archived Revision result or transcript recovery cannot leak into the current task. The legacy Runs API keeps its protocol-response recovery behavior, while the Task read path skips transcript scans and planning-document hydration. Delegated child tasks additionally require the stored parent session, source Run, and action identity to match the canonical delegated Run, and expose only `ActionResult.result` or an explicit fallback summary. Archived history queries return compact metadata, while a version-specific query loads the immutable body and workflow. Database Markdown remains authoritative and is projected atomically to `.harness/sessions/<session_id>/tasks/<task_id>/`; projection reads share the Runs path, symlink, realpath, no-follow, and inode checks, and report hash drift without writing file content back to the database.
