...
Session tree resume has two modes. `restore` restarts the existing loop without adding a user message and is limited to `interrupted`, `queued`, `rate_limited`, `retry`, `running`, and `starting`. Stopped states such as `error`, `failed`, `blocked`, `paused`, `aborted`, and `timeout` require `message` mode so the runtime appends an explicit resume instruction and the model can decide whether to continue or report a blocker. `completed`, `terminal_reply`, and `user_completed` also require `message` mode plus `include_completed`; `archived` is skipped.
