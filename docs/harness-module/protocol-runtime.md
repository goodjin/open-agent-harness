...
After consultation, milestone planner synthesizes one milestone design and sends it to `design-reviewer`. Blocking coverage, consistency, feasibility, dependency, scope, or acceptance findings are resolved before feature child calls are declared. Small low-risk milestones with obvious boundaries and no shared technical decision may skip this council.

Each `feature-planner` handoff carries the milestone goal and exit state, the feature boundary, shared architecture decisions and contracts, relevant ownership and interface boundaries, earlier feature dependencies, integration and rollout assumptions, milestone test constraints, acceptance signals, settled decisions that must be preserved, and unresolved feature-local questions. This keeps feature sessions aligned without making them repeat milestone-level design work.
