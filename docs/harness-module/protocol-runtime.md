...
Delegated task prompts must include a concrete native `ActionResult` argument example. For worker tasks, the example includes `role: "worker"`, the assigned `action_id`, `status`, `result`, and the short string handoff fields. For verifier tasks, the example includes `role: "verifier"`, the verifier `action_id`, the worker `target_action_id`, `status`, `result`, and verification evidence fields. The example is part of the runtime contract because provider tool-call behavior can degrade when the model only receives prose field descriptions.

`role` is required because it is the native `ActionResult` schema discriminator. Runtime uses `role: "worker"` to parse a task handoff result and `role: "verifier"` to parse a verification result that targets another action. Without `role`, the parser cannot know whether `target_action_id`, verifier statuses, worker statuses, or worker handoff fields apply, so the tool call is rejected before delegation state can be updated. When this schema parse fails, the tool error returned to the model must restate the strict direct-argument protocol, the `role` branch meaning, worker/verifier required fields, valid statuses, and an example.
