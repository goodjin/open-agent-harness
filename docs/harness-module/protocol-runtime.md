...
Task action identity is `(run_id, action_id)`. The current read model hydrates at most the latest fifty registered Runs and uses that same sliced Run list for loading, identity tagging, and reduction; actions from older Runs remain in the immutable workflow snapshot but do not enter current actions or progress. Result synchronization is accepted only while the Task is `running`, `waiting_user`, or `revising`, the current Revision is `active`, and the bound Run is still registered. A planned action may upgrade once to a real ResultAction. An identical real replay is idempotent; a divergent real replay or downgrade to a planned action is rejected without changing Task or Revision storage.
