...
The live event stream is an invalidation channel, not the sole source of session truth. Every `server.connected` or `global.disposed` event advances a connection epoch, clears directory session-tree load metadata, and reloads session statuses, roots, and descendants from the server. An open session observes the same epoch and force-refreshes its full detail, latest message page, Todo projection, and Runs descendants. This compensates for `message.updated` or `session.created` events missed during a process restart, network interruption, or background-tab reconnect, so a Turn already claimed by Runtime cannot remain indefinitely rendered from an old `queued` cache entry.
