...
Initial feature development usually belongs in an ordinary outer DAG node. Repair, revision, or retry work caused by feedback belongs inside the loop node. This keeps the global dependency graph analyzable while still allowing bounded runtime feedback.
