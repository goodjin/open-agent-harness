...
| Task | Delivery | Scope | Verification |
|---|---|---|---|
| T-03A1 | architecture | Add `entry` design to agent schema docs. | `docs/harness-protocol/01-agent-model-and-authoring.md` documents `primary`, `delegable`, `mentionable`, `default`, and `hidden`. |
...
| T-0504 | library | Replace tool-definition plugin hook with explicit tool registry extension policy. | Tool registry tests pass without `@open-agent-harness/plugin` runtime calls. |
