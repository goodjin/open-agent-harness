...
This selects the agent for the command; it does not by itself force a child-session invocation.
Use `subtask: true` when you want the command to run as a separate delegated child session.
For compatibility, legacy non-primary/delegation-only agents can still default to this child-session style unless you set `subtask` to `false`.
...
Use the `subtask` boolean to force the command to trigger a delegated child-session invocation.
This is useful if you want the command to not pollute your primary context and will **force** the command to run as a separate child-session invocation.
The target agent must be available for delegation, which is controlled by the agent's entry metadata; legacy `mode` values are mapped to compatible entry defaults.
