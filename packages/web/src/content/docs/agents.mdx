...
Agents can be available through multiple entry points. The main ones are primary usage, delegation, and direct mention.

---

### Primary agents

Agents with primary entry are the main assistants you interact with directly. You can cycle through them using the **Tab** key, or your configured `switch_agent` keybind. These agents handle your main conversation. Tool access is configured via permissions — for example, Build has all tools enabled while Plan is restricted.
...
Delegable agents are specialized assistants that other agents can invoke for specific tasks. You can also manually invoke mentionable agents by **@ mentioning** them in your messages.

OpenCode comes with two built-in delegable agents, **General** and **Explore**. We'll look at this below.

---

## Built-in

OpenCode comes with two built-in primary agents and two built-in delegable agents.

---

### Use build

_Entry_: primary, delegable, mentionable, default
...
2. Delegable or mentionable agents can be invoked:
   - **Automatically** by other agents for specialized tasks based on their capability and descriptions.
   - Manually by **@ mentioning** a mentionable agent in your message. For example.

     ```txt frame="none"
     @general help me search for this function
     ```

3. **Navigation between sessions**: When delegation creates child sessions, use `session_child_first` (default: **\<Leader>+Down**) to enter the first child session from the parent.
...
4. Once you are in a child session, use:
   ...
   This lets you switch between the main conversation and specialized delegated work.
...
:::tip
If you don’t specify a model, primary agents use the [model globally configured](/docs/config#models) while delegated child-session invocations use the model of the agent that invoked them.
:::
...
Control legacy entry behavior with the `mode` config. `mode` is still accepted for compatibility and maps to entry defaults for where an agent can be used.
...
- `primary` maps to primary, delegable, mentionable, and default entry.
- `subagent` maps to delegable and mentionable entry.
- `all` maps to primary, delegable, mentionable, and default entry.

Newer built-in agent templates use explicit `entry` metadata internally. User config can keep using `mode`; OpenCode derives the compatible entry fields for routing and UI filtering.

---

### Capability

Agent templates can also expose capability metadata that describes what the agent is good at. Delegation uses this metadata together with the agent `description` to decide when an agent is a good fit.

Capability metadata is descriptive; it does not grant tool access. Use [permissions](/docs/permissions) to control whether an agent can read, edit, run commands, or invoke other agents.

---

### Hidden

Hide an agent from normal user-facing pickers with `hidden: true`. This is useful for internal agents that should only run through system or programmatic flows.
...
This affects primary switching, mention autocomplete, and normal delegation candidate lists. In entry terms, it behaves like `entry.hidden`.

---

### Task permissions

Control which delegable agents an agent can invoke via the Task tool with `permission.task`. Uses glob patterns for flexible matching.
...
When set to `deny`, the agent is removed from the Task tool description entirely, so the model won't attempt to invoke it.
...
:::tip
Users can always invoke any mentionable agent directly via the `@` autocomplete menu, even if the agent's task permissions would deny it.
:::
