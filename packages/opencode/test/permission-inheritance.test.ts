...
describe("Permission inheritance integration with real permission config", () => {
  ...
  test("subtask session inherits merged permissions from parent", async () => {
    await using tmp = await tmpdir({
      git: true,
      config: {
        permission: {
      ...
      fn: async () => {
        await WorkspaceContext.provide({
          ...
          fn: async () => {
            ...
            // Get permission from config (simulating agent permission)
            const cfg = await Config.get()
            const agentPerm = PermissionNext.fromConfig(cfg.permission ?? {})

            // The merged permission should combine agent permission + session permission
            const merged = PermissionNext.merge(agentPerm, parentSession.permission ?? [])

            // The bash permission from parent session should be preserved
            const bashResult = PermissionNext.evaluate("bash", "ls", merged)
            expect(bashResult.action).toBe("deny")

            // The task permission from agent should be preserved
            const taskResult = PermissionNext.evaluate("task", "general", merged)
            expect(taskResult.action).toBe("allow")
          },
        })
      },
    })
